...
import { getMessages, getSingleMessageById } from "../../../messages/assets/js/messages";
...
//handle show message
const showMessageHandle = () => {
    document.body.addEventListener("click" , async(event) => {

        if(event.target.classList.contains("js-show-message"))
        {
            const messageId = event.target.dataset.id;

            //change watched status

            const message = await getSingleMessageById(messageId);

            console.log(message);

            message.Watched = true;

            const documentationReference = doc(firestore , "Reviews" , messageId);

            try{
                await updateDoc(documentationReference , message);
                location.assign(`/adminPanel/messages/messages?id=${messageId}`);
            }
            catch(error) {
                console.log(error);
            }
