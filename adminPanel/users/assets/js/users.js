    ...
    return new Promise(async(resolve , reject) => {
        ...
        try{
            ...
            if(currentObject.ProfileImage.name)//check if admin sending image
            {
                ...
                //delete old image
                if(oldUser.ProfileImage){ //check if admin was have old image delete it
...
export const changeEmailHandle = (currentObject) => {
    ...
    return new Promise(async(resolve , reject) => {
        try{
            signInWithEmailAndPassword(authetication , authetication.currentUser.email , currentObject.Password)
